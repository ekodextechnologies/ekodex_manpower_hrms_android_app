...
import com.ekodex.manpowerhrms.databinding.FragmentPendingVouchersBinding
...
class PendingVouchersFragment : Fragment() {

    lateinit var binding: FragmentPendingVouchersBinding
    ...
    override fun onCreateView(
        ...
        binding = DataBindingUtil.inflate(inflater, R.layout.fragment_pending_vouchers, container, false)
...
//        binding.pendingVouchersList.adapter = adapter
        ...
        if (adapter == null) {
            adapter = VoucherAdapter(vouchersList,requireActivity()) {
                offset = 0
                vouchersList.clear()
                getPendingVouchers(fromDate, toDate)
            }
        }
        binding.pendingVouchersList.adapter = adapter
        ...
        binding.button26.setOnClickListener {
            if (from_flag == "true" && to_flag == "true") {
                ...
                {
                    Toast.makeText(requireContext(),"Please Check internet connection!!", Toast.LENGTH_LONG).show()
                    binding.pendingVouchersList.visibility = View.INVISIBLE
        ...
        binding.pendingVouchersList.addOnScrollListener(object : RecyclerView.OnScrollListener() {
            override fun onScrolled(recyclerView: RecyclerView, dx: Int, dy: Int) {
                ...
                if (!isLoading && lastVisibleItem >= vouchersList.size - 5 && vouchersList.size < totalRows) {
                    offset += limit
                    getPendingVouchers(fromDate, toDate) // Fetch next page
                }
            }
        })
    ...
    private fun getPendingVouchers(fr:String,too:String) {
        if (isLoading) return
        isLoading = true
      ...
      //  binding.progressBar2.visibility = View.GONE
                ...
                try {
                    ...
                    if (!obj.getBoolean("error")) {
                        ...
                        else
                        {
                            binding.notAvailable1.visibility = View.GONE
                            binding.progressBar2.visibility = View.GONE
                            binding.pendingVouchersList.visibility = View.VISIBLE
        ...
        stringRequest.tag = "pend_vouchers"
        VolleySingleton.getInstance(requireActivity().applicationContext)
            .addToRequestQueue(stringRequest)
    }
    override fun onDestroyView() {
        super.onDestroyView()
        VolleySingleton.getInstance(requireActivity()).requestQueue.cancelAll("pend_vouchers")
    }
...
// Here we can use calendar selected date
  ...
  /*  override fun onResume() {
        ...
        {
            var args = PendingVouchersFragmentArgs.fromBundle(requireArguments())
...
//        if(::fromDate.isInitialized) outState.putString("fromDate", fromDate)
//        if(::toDate.isInitialized) outState.putString("toDate", toDate)
//    }
