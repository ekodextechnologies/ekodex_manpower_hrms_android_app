...
class SupervisorAttendanceFragment : Fragment() {

    lateinit var binding: FragmentSupervisorAttendanceBinding
   ...
   //date filter
    ...
    override fun onCreateView(
        ...
        binding = DataBindingUtil.inflate(
            inflater,
            R.layout.fragment_supervisor_attendance,
            container,
            false
        )
...
// Here we can use calendar selected date
    ...
    private fun splitdata(csvDataResponse: JSONArray) {
        ...
        data.append("Employee Name,Rank,Employee Code,Attendance Date,Status,Check In Time,Check Out Time,Created By")
        ...
        if (array.length() == 0) {
            ...
            for (i in array.length() - 1 downTo 0) {
                ...
                val empName = item.getSafeString("emp_name")
                val rank = item.getSafeString("rank")
                val date = item.getSafeString("date")
                val status = item.getSafeString("status")
                val empCode = item.getSafeString("emp_code")
                val createdBy = item.getSafeString("created_by")
                val checkin_time = item.getSafeString("check_in")
                val checkout_time = item.getSafeString("check_out")
    ...
    fun JSONObject.getSafeString(key: String): String {
        ...
        return if (value.isEmpty() || value.equals("null", true)) "N/A" else value
    }
