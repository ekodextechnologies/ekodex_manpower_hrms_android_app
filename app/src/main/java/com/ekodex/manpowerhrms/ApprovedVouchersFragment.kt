...
import com.ekodex.manpowerhrms.databinding.FragmentApprovedVouchersBinding
...
class ApprovedVouchersFragment : Fragment() {

    lateinit var binding: FragmentApprovedVouchersBinding
    ...
    override fun onCreateView(
        ...
        binding = DataBindingUtil.inflate(inflater, R.layout.fragment_approved_vouchers, container, false)

        if (adapter == null) {
            adapter = VoucherAdapter(vouchersList,requireActivity())
        }
        binding.approvedVouchersList.adapter = adapter
...
//            }
        ...
        binding.button26.setOnClickListener {
            if (from_flag == "true" && to_flag == "true") {
                ...
                {
                    vouchersList.clear()
                    getApprovedVouchers(fromDate, toDate)
                }
                else
                {
                    Toast.makeText(requireContext(),"Please Check internet connection!!", Toast.LENGTH_LONG).show()
                    binding.approvedVouchersList.visibility = View.INVISIBLE
        ...
        binding.approvedVouchersList.addOnScrollListener(object : RecyclerView.OnScrollListener() {
            override fun onScrolled(recyclerView: RecyclerView, dx: Int, dy: Int) {
                ...
                if (!isLoading && lastVisibleItem >= vouchersList.size - 5 && vouchersList.size < totalRows) {
                    offset += limit
                    getApprovedVouchers(fromDate, toDate) // Fetch next page
                }
            }
        })
...
//        })
    ...
    private fun getApprovedVouchers(fr:String , too:String) {
        if (isLoading) return
        isLoading = true
      ...
      //  binding.progressBar2.visibility = View.GONE
                ...
                try {
                    ...
                    if (!obj.getBoolean("error")) {
                        ...
                        else
                        {
                            ...
                            binding.notAvailable1.visibility = View.GONE
                            binding.progressBar2.visibility = View.GONE
                            binding.approvedVouchersList.visibility = View.VISIBLE
        ...
        stringRequest.tag = "appr_vouchers"
    ...
    override fun onDestroyView() {
        super.onDestroyView()
        VolleySingleton.getInstance(requireActivity()).requestQueue.cancelAll("appr_vouchers")
    }
...
/*    override fun onResume() {
        ...
        {
            var args = ApprovedVouchersFragmentArgs.fromBundle(requireArguments())
