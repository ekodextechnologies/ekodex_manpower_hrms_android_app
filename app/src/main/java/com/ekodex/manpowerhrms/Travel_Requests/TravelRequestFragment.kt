...
import com.ekodex.manpowerhrms.Travel_Management.TravelManagementFragmentDirections
import com.ekodex.manpowerhrms.databinding.FragmentTravelRequestBinding
import org.json.JSONException
import org.json.JSONObject

class TravelRequestFragment : Fragment() {

    lateinit var binding: FragmentTravelRequestBinding
    ...
    override fun onCreateView(
        ...
        binding = DataBindingUtil.inflate(inflater,
            R.layout.fragment_travel_request, container, false)
        ...
        else
        {
            ...
            binding.travelRequestsList.visibility = View.INVISIBLE
        ...
        binding.addRequest.setOnClickListener {
            it.findNavController().navigate(TravelManagementFragmentDirections.actionTravelManagementFragmentToTravelcreateRequestFragment())
        }
    ...
    private fun getTravelRequests() {
        var jobsList = mutableListOf<TravelRequestsData>()
        var adapter: TravelRequestsAdapter

        val stringRequest = object : StringRequest(
            Request.Method.POST, URLs.URL_GET_TRAVEL_REQUESTS,
                    ...
                    if (!obj.getBoolean("error")) {
                        ...
                        if (array.length() <= 0) {
                            ...
                            for (i in (array.length() - 1) downTo 0) {
                                ...
                                val banners = TravelRequestsData(
                                    objectArtist.optString("id"),
                                    objectArtist.optString("from"),
                                    objectArtist.optString("to"),
                                    objectArtist.optString("destination")
                                )
                                jobsList.add(banners)
                                // searchList = empDirectoryList as ArrayList<EmployeeDirectoryData>
                                adapter = TravelRequestsAdapter(jobsList)
                                binding.travelRequestsList.adapter = adapter
                                binding.travelRequestsList.visibility = View.VISIBLE
                                binding.notAvailable1.visibility = View.GONE
                            }
                        }
